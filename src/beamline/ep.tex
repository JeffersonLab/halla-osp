...
\footnote{Authors: D.Higinbotham \email{doug@jlab.org} and B.Reitz \email{reitz@jlab.org}}
}
...
Hall A MEDM~\cite{MEDMwww} graphical user interface (EPICS~\cite{EPICSwww}), which runs
...
The controls actually run on a VME computer \mycomp{hallasc6} 
...
moves while it is in the electron beam. \infolevtwo{ To operate the target one has to
...
The Cherenkov detector is filled with pure CO$_2$-gas. \infolevtwo{A schematic of the gas 
...
be at zero. However the flow regulator under the left flow meter must be open.
...
If the gas Cherenkov detector must be opened, one should turn down the gas flow
...
and the volume is closed again, the detector must be set in \mycomp{Flow Mode}.
The left rotary switch must be in the \mycomp{DEBIT} and the right one in the
\mycomp{OUVERT} position, the gas flow regulator must be opened. After the 
...
The data acquisition (DAQ) is runs on \mycomp{adaqep} in the
...
Press \mycomp{p} to show the present setting, it should look as follows:\\
\\
...
To run CODA, you must be logged into \mycomp{adaqep} as user
\mycomp{epmeas}. From the prompt CODA can be started with the
command \mycomp{runcontrol}. Withing CODA you click 
...
and for the Cherenkov photomultiplier tubes muat be turned on (see Sec. 
\ref{sec:ep_highvoltage}). Finally the DAQ should be 
prepared (see Sec. \ref{sec:ep_daq}).

For the eP measurement, the following requirements muat 
...
\input{beamline/ep-personnel}

% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/beamline/ep.tex,v 1.9 2008/05/09 23:08:20 doug Exp $
% $Id: ep.tex,v 1.9 2008/05/09 23:08:20 doug Exp $
% $Author: doug $
% $Date: 2008/05/09 23:08:20 $
% $Name:  $
% $Locker:  $
% $Log: ep.tex,v $
% Revision 1.9  2008/05/09 23:08:20  doug
% eP Safety Updates
%
% Revision 1.8  2005/04/04 22:27:24  gen
% Chnges after the review
%
% Revision 1.7  2004/12/15 00:12:59  gen
