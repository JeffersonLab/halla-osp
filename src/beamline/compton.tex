...
\footnote{
  $CVS~revision~ $Id: compton.tex,v 1.4 2003/11/17 00:55:21 nanda Exp $ $
}
...
\url{http://hallaweb.jlab.org/compton/Documentation/Papers/falletto.pdf}
...
\url{http://hallaweb.jlab.org/compton/Documentation/Papers/neyret.pdf}.
...
\subsubsection{Fast acquisition system}
...
analysis, and the EPICS slow control system. To begin compton polarimeter activity log on to:


\noindent machine:  compton.jlab.org\\
username: compton\\
...
\newpage
\item Click on the "CONNECT" button. You will get this window\\
\begin{figure}[htp]
    \begin{center}
...
\newpage
\item Click on the {\bf Configure} button and you will get the following window :
\begin{figure}[htp]
    \begin{center}
...
configure succeeded"
\item Click on the "Download" Button to get the following messages.
\begin{figure}[htp]
    \begin{center}
...
\item Click on End Run button to stop the acquisition.
...
Don't be surprised. 7 windows will be open but regroup in only one
        ...
        does not go inside spy\_acq window. Click on the corresponding widget.

\item Check the High Voltages applied on the Photon detector PMT's.
Go to the "Acq" panel where spy\_acq is currently runnning,
Go in the Logbook panel and select photon detector. You will get the 
         following panel with the High voltages values:
\begin{figure}[htp]
    \begin{center}
        \includegraphics*[angle=0,scale=0.6]{compton_spy_ht}
    \end{center}
...
Choose the EPICS desktop and in a fresh terminal window and start the MEDM EPICS panel by 
executing  the command:

\$ epicshall

This will open the main EPICS menu for the Compton as shown below:
 \begin{figure}[htp]
%    \begin{center}
        \includegraphics*[angle=0,scale=0.6]{compton_epicsall}
%    \end{center}
...
To turn the Laser On .... Click on the Laser On button.\\
	...
	(second from left among the 4 screens)\\
        and you should see a bright spot on the miror control TV screen 
 ...
 \begin{figure}[htp]
    ...
    \end{center}
    \label{fig:compton_laseroff_photo}
 \end{figure}
...
\item {\bf Lock the cavity}\\
 ...
 To lock the cavity click on the Servo On button
 \begin{figure}[htp]
    \begin{center}
...
on the CCD control TV (as bellow), and you should have more than 1000 W stored
...
If it isn't the case, turn on the Slow Ramp ... Click on the Slow On button ...\\
\begin{figure}[htp]
    \begin{center}
...
\newpage
\subsubsection{Electron Detector Setup}
\begin{itemize}
...
\begin{figure}[htp]
    \begin{center}
        \includegraphics*[angle=0,scale=0.5]{compton_det_elec19}
    \end{center}
    \label{fig:compton_det_elec19}
\end{figure}
...
The electron detector must be on the garage position.
        ...
        Check the status of the electron detector on\\
          the video screen : The arrow must be exactly in front\\
...
\item {\bf Switch on the the Compton chicane}\\
       ...
       The complete procedure is available on the MCC web site at :
http://opsntsrv.acc.jlab.org/ops\_docs/online\_document\_files/MCC\_online\_files/HallA\_beam\_delivery\_proc.pdf
       
\item {\bf Lock once again the cavity}\\
\end{itemize}

\subsubsection{Vertican Scan}
...
The scan is done in contact with MCC (7047) by checking the online evolution
        ...
        on the middle of the spy\_acq picture below ("{\bf Counting Rates versus DAQ Y
...
\begin{figure}[htp]
    \begin{center}
        \includegraphics*[angle=0,scale=0.8]{compton_spy_bell}
    \end{center}
...
by loading the file ComptonElectron.adl located
under the home directory of hacuser on the hac computer.
\begin{figure}[htp]
    \begin{center}
        \includegraphics*[angle=0,scale=0.8]{compton_epicsElectron}
...
logbook by filling up the {\bf LogEntry window} in spy\_acq. Click on 
{\bf Submit} to dowload your comments in the logbook.


\subsubsection{Turning off the compton polarimeter}
\begin{itemize} 
\item  Stop the magnetic chicane\\
       ...
       http://hallaweb.jlab.org/compton/Documentation/Procedures/compton\_off.frm.ps
...
be off (see Hall A run coordinator and call MCC operator) 
If it is not the case the detector will eventually be destroyed.
Slow control of the electron detector. To perform operations on the electron detector. 
...
The list
of the presently authorized personnel is given in Tab.~\ref{tab:compton:personnel}.
Other individuals must notify and receive permission from
the contact person (see Tab.~\ref{tab:compton:personnel}) before adding their names
to the above list.
\begin{table}[ht]
\begin{center}
\begin{tabular}{|ll|l|l|l|l|c|} \hline
  ...
  \cline{4-5}
   &  &   & JLab & Pager &  & \\
\hline
 {\em Nanda}&{\em Sirish }  & JLab    & 7176 & 7176 & nanda@jlab.org      & Contact     \\
 Segal&  Jack    & Jlab &  5849 &  5849     & segal@jlab.org  & Technical \\
 Zhang&  Joseph    & Jlab &  5849 &  5849     & shukui@jlab.org  & Optics \\
Authier& Martial     & CEA & 4324* &      &  mauthier@Cea.Fr & Engineering \\
  Etienne& Burtin      & CEA & 2338* &      & eburtin@Cea.Fr &  \\
 Colombel& Nathalie    & CEA &  8350* &      & ncolombel@Cea.Fr  & Mechanical \\
  Deck& Pascale   & CEA &  2426* &      &  pdeck@Cea.Fr & Electronics \\
  Delbart& Alain   & CEA &  3454* &      & adelbart@Cea.Fr & Optics \\
  Lhuillier& David   & CEA &  9497* &      &  dlhuillier@Cea.Fr & Analysis  \\
  Lussignol& Yves   & CEA  & 2828* &      &  lussi@Cea.Fr & EPICS \\
  Neyret& Damien   & CEA & 7552* &      &  dneyret@Cea.Fr & DAQ \\
  Tarte& Gérard  & CEA &  8464* &      & gtarte@Cea.Fr &  Electronics\\
Veyssière&  Christian &CEA & 9704* & & cveyssiere@Cea.Fr & Electronics \\
\hline
\end{tabular}  
\end{center}
...
% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/beamline/compton.tex,v 1.4 2003/11/17 00:55:21 nanda Exp $
% $Id: compton.tex,v 1.4 2003/11/17 00:55:21 nanda Exp $
% $Author: nanda $
% $Date: 2003/11/17 00:55:21 $
% $Name:  $
% $Locker:  $
% $Log: compton.tex,v $
% Revision 1.4  2003/11/17 00:55:21  nanda
% Compton Polarimeter New
%
