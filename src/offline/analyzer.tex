...
generic and should be able to accomodate most new detectors of the respective 
...
(adaql1--5, adaqs1--3). To run the Analyzer, log into any
standard account on these computers, for example as {\tt adaq},
and type\\
...
the location pointed to by the environment variable {\tt DB\_DIR}.
...
you will need to re-define {\tt DB\_DIR} to
...
the {\tt PATH} and/or {\tt LD\_LIBRARY\_PATH} variables have been
...
\begin{tabbing}
For csh/tcsh shells:$\qquad$ \= {\tt source /adaqfs/apps/env/login.adaq}\\
For bash shells:     \> {\tt source /adaqfs/apps/env/profile.adaq}
\end{tabbing}
...
\noindent{\tt db\_<apparatus-name>.<detector-name>.dat}
\vspace{2ex}
...
create a database subdirectory {\tt $\tilde{}$/DB/20040401} and set
{\tt DB\_DIR=$\;\tilde{}$/DB}. Other files supporting the replay
of this experiment would reside in an experiment-specific replay
directory, usually {\tt \$EXPERIMENT}.
...
{\tt THaPhysicsModule}, which in turn inherits from {\tt THaAnalysisModule}.
...
please contact Ole Hansen at ext.\ 7627, email {\tt ole@jlab.org}.
Bob Michaels (ext.\ 7410, {\tt rom@jlab.org}) 
and Robert Feuerbach (ext.\ 7254, {\tt feuerbac@jlab.org}) 
...
% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/offline/analyzer.tex,v 1.1 2003/11/15 08:56:27 ole Exp $
% $Id: analyzer.tex,v 1.1 2003/11/15 08:56:27 ole Exp $
% $Author: ole $
