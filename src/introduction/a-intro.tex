...
\infoleveqnull{
 This chapter primarily Hall A safety information which is generated
 as a reduced version ({\it ``info level \infolevel{}''}) of the OSP document. 
 This chapter should be
 sufficient to get acquainted with major hazards in Hall A
 and the points of contact.  
 For operation one should read the full ({\it ``info level 4''}) 
 OSP document~\cite{HallAosp} as well as the published article on
 the Hall A base equipment~\cite{HallA-NIM}.   This document does contain active
 links so is best read as a pdf file.
}
