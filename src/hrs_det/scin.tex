...
\begin{figure}[tbh]
  \begin{center}
    \includegraphics[angle=0,width=\textwidth]{scint2_layout}
...
\infolevone{
  ...
  time ( 2.8 ns ) and photo electron statistics the time resolution is about 0.2 ns.
  ...
  a terminal server or through the EPICS system based on the HAC computer.
...
\infolevtwo{
  ...
  \begin{figure}[p]
    \begin{center}
%\begin{latexonly}
      \includegraphics[angle=90.0,height=20cm]{medm_halla_hv_rhrs3}
%end{latexonly}
\html{
      \includegraphics[height=20cm]{medm_halla_hv_rhrs3}
}%
...
%end{latexonly}
\html{
      \includegraphics[height=20cm]{medm_halla_hv_card}
}
...
\infolevone{
  ...
  \subsection{Measures to Protect the PMTs from Helium}
  
  \begin{figure}[tbh]
    \begin{center}
      \includegraphics[width=.8\textwidth]{scint2_housing}
  ...
  problem, each PMT for S1 and S2 is enclosed in a hermetic housing. Air from
  outdoors is supplied at a slight over-pressure. A diagram of the housing
  for the S2 PMTs is shown in Figure~\ref{fig:s2_pmt_housing}.
}

\infolevthree{
  \subsection{ 2" PMT Bases for S1 Trigger Counters}
...
% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/hrs_det/scin.tex,v 1.4 2003/11/11 20:13:58 feuerbac Exp $
% $Id: scin.tex,v 1.4 2003/11/11 20:13:58 feuerbac Exp $
% $Author: feuerbac $
% $Date: 2003/11/11 20:13:58 $
% $Name:  $
% $Locker:  $
% $Log: scin.tex,v $
