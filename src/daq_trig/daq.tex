...
% Date:  March, 1999
% Updated: Aug, 2003
% Updated: Oct 22nd, 2013 Alexandre Camsonne 
...
\par
The public account \mycomp{adaq} is normally
used for running DAQ and \mycomp{a-onl} is used
for running other online software including
the C++ analyzer Podd.
On ``\mycomp{a-onl}'' the directory
tree of an experiment is \mycomp{a-onl/\$EXPERIMENT}
which is organized in subdirectories of 
various tasks, such as scaler display,
Podd, and other online codes, all of
...
The DAQ computer's names are denoted by \mycomp{adaqlN}, where
N is a number.  adaql1 and l2 are Linux computers for 
running DAQ while \mycomp{adaql3,4}... and higher 
are for analysis. A new computer has been installed and will be used for the 
Left HRS DAQ it is named adaq1.
...
spectrometer and the adev account on \mycomp{adaql1} 
...
R-HRS has fastbus crates ROC1 and ROC2 on the lower level
of the detector hut, and a 9U (tall) VME crate TS0 on 
the upper level.  L-HRS has fastbus ROC3, ROC4 and 
VME crate TS11.}
