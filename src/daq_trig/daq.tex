...
\footnote{
  $CVS~revision~ $Id: daq.tex,v 1.4 2003/08/01 20:34:06 rom Exp $ $
}
...
timing; and (3) model 1881M ADCs
for analog signals from scintillators, 
...
\par
The public account ``a-onl'' is normally
used for running DAQ and ``adaq'' is used
...
``atrig'' account and is described in
the Trigger chapter. 
}
...
while adaql3,4... and higher are for analysis.
...
does not want the PEDRUN result), see /ped/README.
...
/adaql2/dataN, N=1,2,3...etc.  Files are 
...
\hskip 0.05in
Edit the file /prescale/ prescale.dat.
...
by logging onto adaql1 as the
adaq account and type ``bta''.
...
\subsection {Port Servers}
Portservers are devices on the network that
allow access to RS232 ports.  Here is how to 
connect from a computer: ``telnet hatsv5 2011''
will connect to the portserver at IP hatsv5
...
as follows ``telnet hatsv5'' as user = root.
At the prompt, type ``kill tty=4'' to clear port 4,
then ``exit''.  Now you can ``telnet hatsv5 2004''.
...
% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/daq_trig/daq.tex,v 1.4 2003/08/01 20:34:06 rom Exp $
% $Id: daq.tex,v 1.4 2003/08/01 20:34:06 rom Exp $
% $Author: rom $
% $Date: 2003/08/01 20:34:06 $
% $Name:  $
% $Locker:  $
% $Log: daq.tex,v $
