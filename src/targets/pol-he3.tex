...
%\footnote{
%  $CVS~revision~ $Id: pol-he3.tex,v 1.12 2005/04/04 22:27:25 gen Exp $ $
%}
...
The polarized $^3$He target system provides a high-density 
($\approx 2.5\times 10^{20}$ nuclei/cm$^3$)
polarized $^3$He gas target for spin physics experiments. 
The target cells contain high pressure $^3$He gas and a small amount of
rubidium vapor. In addition, they
also contain a small amount of nitrogen to increase the
pumping efficiency.

Polarization is acieved via
the so-called spin-exchange technique.
...
In addition to the target cell,
the polarized target comprises several
components (see Fig.~\ref{fig:overview})
related to its operation:

\begin{enumerate}

\item 
A double set of Helmholtz coils which  provide the target spins with a
...
\item 
A laser shielding house, known as the laser hut, where two laser beam lines
...
and the EPR polarimetry runs under LabView on a PC.
...
on disk and selected information passed to the Hall A event data stream.
...
of the target consists of monitoring the temperatures of the pumping
cell, target cell and reference cell. The temperature in the pumping
cell provides feedback for a temperature controller.  A spectral-analyzer
monitors the laser wavelength and the relative intensity.
When operating in this mode, all the  
electron beam and laser beam interlocks should be on.

\item 
The electron and laser beam are turned off (the laser beam should be turned off first)
and the target is moved to ``Pickup Coil Position''
(polarized $^3$He target in a position
between the NMR pickup coils) and a polarization measurement is
performed using the NMR (AFP) method.  
If the polarization is in the transverse mode, 
...
Once in position,
the laser beam will be turned on. The beam interlock should be
...
light, all laser systems are located in an enclosed laser hut.
...
The seven infrared diode lasers (four for longitudinal pumping and three
...
Once both beams from each laser have identical linear
...
This light will also be viewed by a fiber and enters 
a spectral-analyzer, which is used to monitor the spectrum and intensity of 
the lasers.
...
The laser light is provided by seven 30 Watt, 795
...
two diode lasers made by the Opto-Power Corporation that are used
as spares.  Though there are some minor differences, both companies'
laser systems perform the same function. Only the Coherent lasers and
...
divergence is $< 0.20$ N.A.\footnote{N.A. stands for Numerical Aperture
of the fiber, equal to $\sin{(\theta{}/2)}$, where $\theta{}$ is 
...
\item Apply the safety measures described in Section~\ref{sec:lassaf}.
...
has remote readback.  An alarm will be triggered when the readback deviate
from the regular running condition by more than $10\%$. This enables us 
to know when a laser goes off or when a cell ruptures.
...
The half-wave plate positions are controlled through a MEDM GUI running
...
should be fine except in special circumstances.  
%The GUI allows 
...
The target cell is either a 25 cm or 40 cm long aluminosilicate glass (GE180)
 high pressure (up to 15 atm) double chamber cell. Typical
dimensions for a 40 cm cell are shown in Fig. \ref{fig:cell1}. A 25 cm cell 
is identical except the target chamber is 25 cm long instead of 40 cm.  
It is a closed system filled with a gas mixture which consists of $^3$He, rubidium and
nitrogen. 

The cell volumes range from 160 ml for a 25 cm cell to 200 ml for a 40 cm cell.
The interior pressure of the cell at room temperature is between 8 and 10 
...
of Rb in the pumping chamber.  The tritium contamination of the $^3$He gas 
...
Radiation Worker II training.  Only the installation prior to beam delivery 
does
...
assembly; and third, it allows the region around the target to be filled
...
several removable
windows.  The incident electron beam
...
controller. The system is described in detail in Section~\ref{sec:polheldesc}.

%----------------------------------
\subsection{Description}
\label{sec:polheldesc}
...
continuous temperature of at least 260$^\circ$C.
...
\item Verify the alarm set points for the flow meter and
  ...
  the setpoint value will appear. It should be 250 for the
...
\item Check correct operation of the controller. A little green 
  ...
  to higher values over short periods of time as the heater 
...
The Helmholtz coils are four large interlocking rings of coils that
provide a large constant magnetic field.  Two sets are necessary so
that the magnetic field can point in any direction in the horizontal
plane.  The larger two coils have an internal diameter of 1.45 m and
consist of 272 turns of coil.  The smaller set of coils each have a
internal diameter of 1.27 m and are made of 256 turns of coil.

The normal holding field for the target is 25 Gauss which corresponds
to approximately 7.2 amps of current in the coils.  However, when
doing an NMR measurement the field gets as high as 32 Gauss, which
corresponds to 9.2 amps of current.

The coils are controlled by a DC voltage from two Stanford Research
...
function generator : sometimes the voltage drops for a short time,
...
Attached to the small set of Helmholtz coils are a smaller set of
 coils.  These consist of 20 loops of 16 AWG wire attached to a 150 W
 power supply.  This target occasionally causes a problem 
 known as masing.  This is a loss of target polarization due
 ...
 disappear.  These coils are used to introduce that field gradient.
...
polarized $^{3}$He target. Please, refer to Fig. \ref{fig:nmr}  
...
\begin{figure}
\begin{center}
\centerline{\includegraphics[scale=0.8]{coils_new}}
...
\begin{figure}
\begin{center}
\centerline{\includegraphics{electronics_may14}}
...
\begin{figure}
\begin{center}
\centerline{\includegraphics[scale=0.75]{NMR_system}}
...
Basically, adjust these parameters such that the feedback signal is not too strong or
too weak. \\
...
manifold, which is located in the electronics rack behind the laser
hut, and a series of gas lines that connect the manifold to the
...
The gas handling system consists of 8 solenoid-controlled,
...
\begin{figure}
%  \begin{center}
  \centerline{\includegraphics[width=\textwidth]{laser_hall}}
...
power of approximately 30 W. The laser beam will already have been aligned, properly 
...
% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/targets/pol-he3.tex,v 1.12 2005/04/04 22:27:25 gen Exp $
% $Id: pol-he3.tex,v 1.12 2005/04/04 22:27:25 gen Exp $
% $Author: gen $
% $Date: 2005/04/04 22:27:25 $
% $Name:  $
% $Locker:  $
% $Log: pol-he3.tex,v $
% Revision 1.12  2005/04/04 22:27:25  gen
