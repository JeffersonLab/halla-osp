...
{The enclosure of the target used for E00-102 and E94-107 
        is shown on the left. The waterfall target within the enclosure
...
\infolevtwo{ (see Fig.\ref{fig:wt_GUI})}%
 accessible from the standard Hall A controls screen.
...
The target consists of a positioning system that places one of 5 targets
...
In particular, one should make sure that the position
encoder does indicate the proper target (there is some acceptable
slop on this number).
} %infolev
\infolevtwo{
...
The target position is selected on the left side of the GUI under Position
...
desired position, the user should see the ``moving'' LED turn yellow
in the Status display. The graphic display to the right of the selection
boxes should update; the blue diamond indicating the current position
of the target. In addition, the ``Encoder Position'' and ``StepMot
Position'' displays to the right of the GUI should update indicating
...
position should be close to the value in text next to the target button.
...
marked with black pen labeled B1. This switch must be set to OFF to
