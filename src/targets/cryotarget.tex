...
\par
\begin{safetyen}{10}{5}
When full power beam with tiny beam spot hits the cryotarget, there is a danger 
...
out once per shift} and stored in the target checklist binder.
...
\item[{\bf High Power Heater}]There are two kapton enclosed incoloy\texttrademark{} heater foils
...
a failure of one (or more) of the pressure transducers. 
If more than one deviates significantly from the normal operating pressure, it could be due to 
a temperature change or could be, in a more serious situation, a leak in the system.
...
IOC is rebooted. After the IOC reboot, the high power heater will be reset 
to zero before going back to PID control. Although the time the high power 
heater is zero is short (for about 1 minute), the temperature will drop.
...
replace the regular one to keep the temperature from dropping.
...
also require the Hall A 
%Hewlett Packard, HP, 
computer for control (HAC, for example \mycomp{hacsbc2} -see Sec.~\ref{chap:controls}) 
...
% ===========  CVS info
% $Header: /group/halla/analysis/cvs/tex/osp/src/targets/cryotarget.tex,v 1.12 2005/04/04 22:27:25 gen Exp $
% $Id: cryotarget.tex,v 1.12 2005/04/04 22:27:25 gen Exp $
% $Author: gen $
