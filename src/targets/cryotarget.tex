...
\infoleveqnull{\subsection{Introduction}}
...
The Hall A cryotarget system contains three target loops. 
One of the loops
often has a single helium cell, which will be filled with 
either 
$^3$He or $^4$He gas with pressure up to 15 atm (about 220 PSIA).
The other two loops are usually used for liquid hygrodgen and deuterium 
targets. Each loop can have two target cells in one loop with different 
sizes, for example, one 15 cm and one 4 cm cells. Or it can have one cell
with length range from 4 cm to 25 cm. The loop not in use  
will usually be filled with a little over one atm helium gas. 
For the next round of cryotarget experiments (DVCS and GMp), all three loops
will have single 15-cm cell for liquid hydrogen target.

During the normal operation, the hydrogen target should
have already been liquefied and are in a stable state of 
about 2 to 3 degree sub-cooled liquid (19K). 
...
to 1000 Watts). The PID loops read the output of one
of the temperature sensors and adjust the power in the high 
...
\infolevltone{
Once the Jefferson Lab target group finished cooling down the Hall A
cyrotarget system, there must be a trained target operator in the counting
house monitoring the system at all times.
For details of the target operation consult the full Hall A operations manual~\cite{HallAosp}.
}
...
\begin{safetyen}{0}{0}
It is \emph{mandatory} to have an alarm handler,ALH, running at all times when 
the target has been cooled-down. 
Further,
...
\begin{safetyen}{0}{0}
If the alarm handler alarms repeatedly or the cause of the alarm is not clear, the 
...
In the case of Gmp experiment, a separate pointing target
is planned to be inside the scattering chamber. An interlock system needs to be
in place to prevent the pointing target to interfere with the cryotarget. The safety aspect of the pointing target needs to be covered in a separate safety document. For the cryotarget operation, it is important to have extra caution to prevent 
the interfereance of cryotarget with the pointing target. 
}
...
is on and has a reasonable size for beam current above 5 $\mu$A.
\end{safetyen}
...
target checklist \emph{must be Elog the charts and the main target page
once per shift}.
}
...
\begin{safetyen}{0}{0}
\subsection{Fire/Explosion Hazards}
...
\item[{\bf High Power Heater}]There is a high-power heater in the pipe of the
loop.
The maximum power available is 1000 $W$. The heater 
is driven by a 150 $V$, 7 $A$ power supply.
...
\item[{\bf Resistive temperature sensors}] There are four
resistive temperature sensors immersed in each target loop. These resistors provide temperature
...
on the order of 30 mV). The resistors are connected to the outside world
...
in the hydrogen system. The outer wall is made of 0.006 $in$ thick aluminum.
The entrance and exit windows are thinner, but no less than  
0.003 $in$.
There is one 15 $cm$ long  cell bolted on to each cell block. 
The cell has an outer diameter of 
approximately 3 inches.
The upstream windows are connected 
to 1.25 $in$ diameter
tubes with flanges which are also 
bolted on to the cell block. A vertical flow diverter plays a role to make the coolant flow in vertical direction to help remove the beal heating effectively.
...
Each target contains four quality temperature measurements with
resistive temperature sensors. The 
temperature regulation is done with a software control of a high power
heater
to the temperature read by one of the sensors.
...
but rather an one for one replacement of the beam load should the beam
...
\subsection{Oxygen Deficiency Harard}
\label{sec:targ:odh}
...
Limit switches are installed to not allow the target to move outside
the allowable range.  
\end{description}
...
\infolevone{\section[Cryogenic Target Control System User Manual]
{Cryogenic Target Control System User Manual 
}}
A short version of 
the cryotarget target control system user manual is available at
\url{http://hallaweb.jlab.org/equipment/targets/cryotargets/Halla_tgt.html}.
An updated User's Guide to the Hall A Cryotarget
...
\begin{itemize}
\item Target has completed cool-down, at least one loop has liquid hydroge with temperature stable at 19K, pressure stable at around 25 psi. 
...
\item All other loops are filled with over 1 atm helium gas.
...
\subsection{Responsible Personnal}
...
\begin{namestab}{tab:cryotarg:personnel}{Cryo Target: experts}{%
   Cryo target: experts and authorized personnel, with their phone numbers}
   \namestabheader{Hall A Physicists} 
   \JianPingChen{cell:218-0722}
   \JohnLeRose{cell:565-5060}
   \namestabheader{JLab Cryo-Target Group} 
   \DaveMeekins{cell:968-9076}
   \ChristopherKeith{cell:746-9277}
\end{namestab}
